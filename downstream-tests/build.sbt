...
ThisBuild / organization := "scalacss-test"
...
lazy val jvm = project
  .in(file("jvm"))
  .configure(commonSettings, utestSettings.jvm)
  .settings(
    ...
    Test / javaOptions ++=
      sys.props.iterator
        .filter(_._1.matches("(downstream_tests|japgolly|scalacss).*"))
        .map(x => s"-D${x._1}=${x._2}")
        .toSeq,
    Test / javaOptions += {
...
lazy val js = project
  ...
  .configure(commonSettings, utestSettings.js)
  .settings(
